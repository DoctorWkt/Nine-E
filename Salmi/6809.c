...
struct watchpoint *watchhead= NULL;

unsigned X, Y, S, U, cPC;	// PC now cPC as conflicts with a library fn
unsigned A, B, DP;
unsigned H, N, Z, V, C;
unsigned EFI;
...
// Print out a called address and some of the stack as debug output
void printcall(char *how) {
  unsigned stk= S;
  if (debugout!=NULL) {
    fprintf(debugout, "%s to %04X: X=%04X, D=%04X, ", how, cPC, get_x(), get_d());
    for (int i=0; i<16; i++) fprintf(debugout, "%02X ", memory[stk++]);
    fprintf(debugout, "\n");
    fflush(debugout);
  }
}
...
static void WRMEM (unsigned addr, unsigned data)
{
  ...
  // Do not write to ROM addresses
  if (addr < romaddr)
    memory[addr] = (UINT8)data;

  // Fall into the monitor if we are writing to a watchpoint
  if (watchhead != NULL) {
...
void puls (void)
{
  ...
  if(post & 0x80) {
    cpu_clk -= 2; cPC  = read_stack16(S);     S = (S + 2) & 0xffff;
    if (debugout!=NULL)
      fprintf(debugout, " RTS to %04X X=%04X D=%04X\n", cPC, get_x(), get_d());
  }
}
...
void rti (void)
{
  ...
  cpu_clk -= 6;
  newEFI= read_stack(S); S = (S + 1) & 0xffff;
...
void rts (void)
{
  cpu_clk -= 5;
  cPC  = read_stack16(S);     S = (S + 2) & 0xffff;
  if (debugout!=NULL) fprintf(debugout, " RTS to %04X X=%04X, D=%04X\n",
						cPC, get_x(), get_d());
}
...
void swi (void)
{
#ifdef USE_LINUX_SYSCALLS
  int sarg1, sarg2, sarg3;
  void *ptr1, *ptr2;
  off_t pos;
  int newbrk;

  // Perform known system calls
  switch(X) {
    case 1:				// exit()
	reset_terminal_mode();
	shutdown_fs();
	exit(RDMEM16(get_s()+2));
	return;
    case 3:				// read()
	sarg1= RDMEM16(get_s()+2);
	ptr2=  (void *) &memory[ RDMEM16(get_s()+4) ];
	sarg3= RDMEM16(get_s()+6);
	set_d( read(sarg1, ptr2, sarg3) );
	set_x(errno);
	return;
    case 4:				// write()
	sarg1= RDMEM16(get_s()+2);
	ptr2=  (void *) &memory[ RDMEM16(get_s()+4) ];
	sarg3= RDMEM16(get_s()+6);
	set_d( write(sarg1, ptr2, sarg3) );
	set_x(errno);
	return;
    case 5:				// open()
	ptr1=  (void *) &memory[ RDMEM16(get_s()+2) ];
	sarg2= RDMEM16(get_s()+4);
	sarg3= RDMEM16(get_s()+6);
	set_d( open(ptr1, sarg2, sarg3) );
	set_x(errno);
	return;
    case 6:				// close()
	sarg1= RDMEM16(get_s()+2);
	...
	return;
    case 17:				// myputchar()
	putchar(RDMEM16(get_s()+2));
	return;
    case 18:				// myprintint()
	// Deal with two's complement 16-bit nums
	sarg1=RDMEM16(get_s()+2);
	if (sarg1>32767)
	  sarg1= sarg1 - 65536;
	printf("%d", sarg1);
	return;
    case 19:				// lseek(). The second
	sarg1=RDMEM16(get_s()+2);	// arg is an off_t pointer
	ptr2=  &memory[ RDMEM16(get_s()+4) ];
	sarg3= RDMEM16(get_s()+6);

	// Ensure pos has correct endianness, as 6809 is big-endian
	pos= ntohl(* (off_t *)ptr2);
	pos= lseek(sarg1, pos, sarg3);
	* (off_t *)ptr2= htonl(pos);
	...
	return;
    case 69:				// sbrk()
	sarg1=RDMEM16(get_s()+2);	// Get the change
	newbrk= curbrk + sarg1;		// Calculate the new break value

	// XXX: Record the highest load value when loading the binary
	// If the new break value is below zero or too close to the
	// current stack pointer value, it's an error
	if ((newbrk < 0) || (newbrk > (get_s() - 8))) {
	  set_d(-1);
	  set_x(ENOMEM);
	} else {
	  set_d(curbrk);
  ...
  // Unknown system call, so let the SWI handler deal with it
  cpu_clk -= 19;
  EFI |= E_FLAG;
  S = (S - 2) & 0xffff; write_stack16(S, cPC & 0xffff);
...
void sync (void)
{
  cPC--; doing_sync=1;	// Keep doing this instruction
  cpu_clk -= 4;
}
...
int cpu_execute (int cycles)
{
  ...
  do
  {
    ...
    if (monitor_on != 0) {
      reset_terminal_mode();		// Go back to blocking I/O
      monitor_result= monitor6809();
      ttySetCbreak();			// Set cbreak mode again
      if (monitor_result != 0) return cpu_period - cpu_clk;
    }
    ...
    // If the keyboard has a character and we're
    // not servicing an IRQ already, do an IRQ.
    if (kbhit() && ((EFI & I_FLAG)==0)) {

      // Decrement irqdelay. Only do and IRQ if it's zero.
      // This gives the monitor code some time to drain
      // the last character read before getting the next one
      irqdelay--;
      if (irqdelay==0) {
        // Stop doing the SYNC if we are doing it
        if (doing_sync) {
	  doing_sync=0; cPC++;
        }
        irq();
...
void cpu_reset (int start_addr, int start_stack)
{
  X = Y = U = A = B = DP = 0;
  H = N = V = C = 0; Z = 1;
  EFI = F_FLAG|I_FLAG;
  ...
  if (start_addr >=0)
    ...
    cPC = (memory[0xfffe] << 8) | memory[0xffff];
